...
class OCRActivity : AppCompatActivity() {
    ...
    private fun takePhotoAndRecognize() {
        ...
        Toast.makeText(this, "正在讀取文字...", Toast.LENGTH_SHORT).show()
        ...
        imageCapture.takePicture(
            ...
            object : ImageCapture.OnImageCapturedCallback() {
                ...
                override fun onError(exc: ImageCaptureException) {
                    ...
                    Toast.makeText(this@OCRActivity, "拍攝失敗", Toast.LENGTH_SHORT).show()
                }
            }
        )
    }
    ...
    private fun processImageProxy(imageProxy: ImageProxy) {
        ...
        recognizer.process(image)
            .addOnSuccessListener { visionText ->
                val resultText = visionText.text.trim().replace("\n", "")
                Log.d(TAG, "OCR Success Result: '$resultText'")
                if (resultText.isNotEmpty()) {
                    OCRResultHolder.pendingResult = resultText
                    finish()
                } else {
                    Log.w(TAG, "OCR Success but text was empty")
                    Toast.makeText(this, "方框內找不到文字，請再對準一點", Toast.LENGTH_LONG).show()
                }
            }
    ...
    override fun onRequestPermissionsResult(
        ...
        if (requestCode == REQUEST_CODE_PERMISSIONS) {
            if (allPermissionsGranted()) {
                ...
                Toast.makeText(this, "需開啟相機權限才能辨識文字，請在設定中開啟", Toast.LENGTH_LONG).show()
                openAppSettings()
                finish()
            }
        }
    ...
    companion object {
        ...
        const val ACTION_OCR_RESULT = "com.wade.fsime.OCR_RESULT"
