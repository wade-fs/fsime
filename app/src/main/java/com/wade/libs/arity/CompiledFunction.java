...
public class CompiledFunction extends ContextFunction {
    ...
    int execWithoutCheck(EvalContext context, int p) throws IsComplexException {
        ...
        for (int pc = 0; pc < codeLen; ++pc) {
            ...
            case VM.SEC:  s[p] = MoreMath.sec(s[p]); break;
            case VM.CSC:  s[p] = MoreMath.csc(s[p]); break;
            case VM.COT:  s[p] = MoreMath.cot(s[p]); break;
            ...
            case VM.ASEC: s[p] = MoreMath.asec(s[p]); break;
            case VM.ACSC: s[p] = MoreMath.acsc(s[p]); break;
            case VM.ACOT: s[p] = MoreMath.acot(s[p]); break;
            case VM.SINH: s[p] = Math.sinh(s[p]); break;
            case VM.COSH: s[p] = Math.cosh(s[p]); break;
            case VM.TANH: s[p] = Math.tanh(s[p]); break;
            case VM.SECH: s[p] = MoreMath.sech(s[p]); break;
            case VM.CSCH: s[p] = MoreMath.csch(s[p]); break;
            case VM.COTH: s[p] = MoreMath.coth(s[p]); break;
            ...
            case VM.ASECH: s[p] = MoreMath.asech(s[p]); break;
            case VM.ACSCH: s[p] = MoreMath.acsch(s[p]); break;
            case VM.ACOTH: s[p] = MoreMath.acoth(s[p]); break;
    ...
    int execWithoutCheckComplex(EvalContext context, int p, int percentPC) {
        ...
        for (int pc = 0; pc < codeLen; ++pc) {
            ...
            case VM.SEC:   s[p].sec();   break;
            case VM.CSC:   s[p].csc();   break;
            case VM.COT:   s[p].cot();   break;
            case VM.SINH:  s[p].sinh();  break;
            case VM.COSH:  s[p].cosh();  break;
            case VM.TANH:  s[p].tanh();  break;
            case VM.SECH:  s[p].sech();  break;
            case VM.CSCH:  s[p].csch();  break;
            case VM.COTH:  s[p].coth();  break;
            ...
            // 底下這六個，針對複數都沒有定義, 請見 Complex.java
            case VM.ASEC:  s[p].asec();  break;
            case VM.ACSC:  s[p].acsc();  break;
            case VM.ACOT:  s[p].acot();  break;
            case VM.ASECH: s[p].asech(); break;
            case VM.ACSCH: s[p].acsch(); break;
            case VM.ACOTH: s[p].acoth(); break;
